...
describe('Patrimony Service & Routes', () => {
  ...
  describe('User Patrimony Calculation', () => {
    ...
    test('should calculate patrimony for user with partial ownership', async () => {
  ...
  describe('Company Patrimony', () => {
    test('should calculate company patrimony with partner participation', async () => {
  ...
  describe('Patrimony Routes', () => {
    ...
    test('GET /patrimony/user/:userId should return user patrimony or skip if auth fails', async () => {
    ...
    test('GET /patrimony/partners should return all partners patrimony or skip', async () => {
      if (authTokens.accessToken === 'mock-token') {
        console.log('Pulando teste de rota HTTP - auth tokens indisponíveis')
        return
      }
    ...
    test('GET /patrimony/investors should return all investors patrimony or skip', async () => {
      if (authTokens.accessToken === 'mock-token') {
        console.log('Pulando teste de rota HTTP - auth tokens indisponíveis')
        return
      }
    ...
    test('GET /patrimony/report should return complete patrimony report or skip', async () => {
      if (authTokens.accessToken === 'mock-token') {
        console.log('Pulando teste de rota HTTP - auth tokens indisponíveis')
        return
      }
    ...
    test('GET /patrimony/stats should return patrimony statistics or skip', async () => {
      if (authTokens.accessToken === 'mock-token') {
        console.log('Pulando teste de rota HTTP - auth tokens indisponíveis')
        return
      }
    ...
    test('POST /patrimony/refresh-cache should refresh FIPE cache or skip', async () => {
      if (authTokens.accessToken === 'mock-token') {
        console.log('Pulando teste de rota HTTP - auth tokens indisponíveis')
        return
      }
  ...
  describe('Multiple Vehicles and Shared Ownership', () => {
    test('should calculate patrimony with multiple vehicles and shared ownership', async () => {
      ...
      const partnerResult = await patrimonyService.calculateUserPatrimony(testPartner.id)
      expect(partnerResult.total_patrimony).toBe(80000) // 50.000 + (50% de 60.000)
      expect(partnerResult.vehicles).toHaveLength(2)

      // Testar cálculo para o investor
      const investorResult = await patrimonyService.calculateUserPatrimony(testInvestor.id)
